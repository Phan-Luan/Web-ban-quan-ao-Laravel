...
@section('content')
    <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
        ...
        <button type="button" class="btn btn-primary clickmodal" data-bs-toggle="modal" data-bs-target="#AddSizeModal">
            Add size
        </button>
        <div class="modal" id="AddSizeModal" tabindex="-1" aria-labelledby="AddSizeModalLabel" aria-hidden="true">
        ...
        <div class="form-group">
            ...
            <select name="category_id" id="" class="form-select">
                ...
                @foreach ($categories as $item)
                    <option value="{{ $item->id }}" {{ $item->id == $product->category_id ? 'selected' : false }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
        </div>
